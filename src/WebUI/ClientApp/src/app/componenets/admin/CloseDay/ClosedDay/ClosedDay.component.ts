...
@Component({
  selector: "app-ClosedDay",
  templateUrl: "./ClosedDay.component.html",
  styleUrls: ["./ClosedDay.component.sass"],
})
export class ClosedDayComponent implements OnInit {
  selected = [];
  fh: FormHelper;
  IsCheckAll: false;
  filteredData = [];
  ...
  formCaption = "New Closed Booking";
  ...
  GetBookingList() {
    this.bookingClient.getClosedBooking(0).subscribe(
      (result: any) => {
        this.selected = [];
        this.dt_datas = [];
  ...
  AddNewBooking(template: TemplateRef<any>, resetModal: boolean = true) {
    if (resetModal) {
      this.bookingId = 0;
      this.roomForm.reset();
      this.formCaption = "New Closed Booking";
    }
  ...
  EditBooking(itemData: any, template: TemplateRef<any>) {
    ...
    this.formCaption = "Edit Closed Booking: " + itemData?.description ?? "";
